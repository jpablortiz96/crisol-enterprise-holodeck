  ...
  return (
    <section className="war-panel report-panel p-5">
      <div className="panel-header !mb-0 !p-0">
        <div>
          <p className="panel-kicker">Cited evidence trail</p>
          <h2 className="panel-title">Competence Report</h2>
        </div>
      ...
      {!report ? (
        ...
        <div className="report-panel-scroll mt-5 space-y-6">
          ...
          <p className="border-l-2 border-cyan-300/60 bg-cyan-300/[0.025] p-4 text-sm leading-6 text-slate-300">
          ...
          <div>
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {Object.entries(report.dimensions).map(([id, dimension]) => (
                <div key={id} className="border-t border-white/5 pt-3">
                  ...
                  <div className="mt-2 h-1 rounded-full bg-white/5">
                    <div className="h-1 rounded-full bg-emerald-300" style={{ width: `${dimension.score}%` }} />
                  </div>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <Panel title="Evidence trail" icon={<FileText className="h-4 w-4" />}>
              {report.evidence_trail.slice(0, 5).map((item) => (
                <div key={item.evidence_id} className="border-l-2 border-cyan-300/30 bg-black/10 p-3">
            ...
            <Panel title="Skill gaps" icon={<ClipboardCheck className="h-4 w-4" />}>
              {report.skill_gaps.slice(0, 5).map((gap) => (
                <div key={`${gap.skill_id}-${gap.dimension_id}`} className="border-l-2 border-amber-300/30 bg-black/10 p-3">
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            ...
            <Panel title="Next best actions" icon={<FileText className="h-4 w-4" />}>
              {report.next_best_actions.slice(0, 5).map((action) => (
                <div key={action.action_id} className="border-b border-white/5 p-3">
